...
import { AlertTriangle, ChevronDown, ChevronUp, Clock, Code2, FileText, Loader2, CalendarDays, Sparkles } from 'lucide-react';
...
export function ErrorDetails({ error, index, isExpanded = false, onToggle, categoryNameMap }: ErrorDetailsProps) {
  ...
  const handleAIAnalysis = async () => {
    ...
    try {
      ...
      setAiSuggestion(
        analysis.errorAnalysis[0]?.suggestion ||
  ...
  return (
    <Card className="border rounded-lg overflow-hidden shadow-sm hover:shadow-md transition-shadow bg-background">
      ...
      <CardContent className="p-4">
        <Collapsible open={isOpen} onOpenChange={handleToggle} className="space-y-3">
          <div className="flex items-center justify-between">
            <Button
              ...
              disabled={isLoadingAI || !!aiSuggestion}
            >
              {isLoadingAI ? (
                <>
                ...
                <>
                  ...
                  <span>{aiSuggestion ? 'Analisado' : 'Analisar com IA'}</span>
                </>
              )}
            </Button>
          ...
          <CollapsibleContent className="mt-4 space-y-4 animate-collapsible">
                ...
                <ScrollArea className="h-[120px] rounded-lg border bg-muted/5 p-3">
                  <div className="space-y-1.5">
                    {error.contextBefore.map((line, i) => (
                      <p key={i} className="text-xs font-mono whitespace-pre-wrap text-muted-foreground">{line}</p>
                    ))}
                  </div>
                </ScrollArea>
              </div>
            ...
            <div>
              ...
              <div className="rounded-lg border border-destructive/20 bg-destructive/5 p-3">
                <p className="text-xs font-mono text-destructive whitespace-pre-wrap">{error.message}</p>
